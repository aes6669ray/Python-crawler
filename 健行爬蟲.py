...
print("請依照以下規則輸入,如:112學年度第1學期,則輸入1121;111學年度第5學期,則輸入1115")
...
if str(smtime) not in df_smtr["學期代碼"].values:
    print("======================輸入學期錯誤======================")
...
for dep_i in df_dept["科系代碼"]:
    ...
    for degre_i in degree:
        ...
        for clas_i in clas:
            creurl="https://cos.uch.edu.tw/course_info/student_class/courselist.aspx?dept="+str(dep_i)+"&degree="+degre_i+"&cos_class="+clas_i+"&smtr="+str(smtime)
...
df=df[["科系","班級","課程名稱","老師"]]
df.to_excel(str(smtime)+"_健行課表.xlsx",index=False)